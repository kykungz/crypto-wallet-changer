...
const bookmarklets = readdirSync(path.join(__dirname, "bookmarklets")).map(
  (filePath) => require(path.join(__dirname, "bookmarklets", filePath))
);
...
const html = dedent/* html */ `
  ...
  <body>
    ...
    <div class="container">
      ${bookmarklets
        .map((bookmarklet) => {
          const { name, logo, script } = bookmarklet;
          const encodedScript = encodeURIComponent(script.toString());

          return /* html */ `
            <a href="${`javascript:(${encodedScript})()`}"
              class="bookmarklet">
